...
class FmovieZ:
    ...
    def get_season_number(self) -> str:
        season_str = self.film["post_title"]
    ...
    def get_year_from(self, released: str) -> int:
        try:
            released = [x for x in released if x.isdigit()]
    ...
    def get_episode_server_from(self, link: str) -> list:
        res = [
            {
                "server_name": CONFIG.CUSTOM_SERVER_NAME,
                "server_type": "embed",
                "server_link": link,
            }
        ]
    ...
    def get_episode_data(self) -> list:
        ...
        for ep_name, ep_link in episodes.items():
            ...
            res.append(
                {
                    ...
                    # "ep_time": 0,
                    "episode_server": self.get_episode_server_from(ep_link),
                }
            )
    ...
    def insert_episodes(self, movie_id: int) -> None:
        ...
        if self.film["post_type"] == CONFIG.TYPE_MOVIE:
            ...
            be_season_data = database.select_or_insert(
                table="season",
                condition=f"movieId={movie_id} AND num={self.film['season_number']}",
                data=(
            ...
            season_id = be_season_data[0][0]

        for ep_name, ep_link in self.episodes.items():
            ep_num = self.get_ep_num_from(ep_name=ep_name)
            data = [
                {
                    "server_name": CONFIG.CUSTOM_SERVER_NAME,
                    "server_link": ep_link,
            ...
            if self.film["post_type"] == CONFIG.TYPE_MOVIE:
                ep_name = self.film["post_title"]

            condition = f"movieId={movie_id} AND seasonId={season_id} AND num={ep_num}"
            be_episode_data = database.select_or_insert(
                table="episode",
                condition=condition,
                data=(movie_id, ep_num, season_id, ep_name, json.dumps(data)),
            )
